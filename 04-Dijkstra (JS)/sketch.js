...
function setup() {
    ...
    searchAlgo = new Dijkstra(blocks, blocks[source.x][source.y]);
}
...
function draw() {
    ...
    if (startAlgo) {
        // Step by Step
        // const path = searchAlgo.start(blocks, blocks[source.x][source.y]);
        const path = searchAlgo.step_start(blocks, blocks[source.x][source.y]);
