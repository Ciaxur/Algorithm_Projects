...
function setup() {
    ...
    searchAlgo = new Greedy(blocks,
        blocks[source.x][source.y],
        blocks[target.x][target.y]);
}
...
function draw() {
    ...
    if (startAlgo) {
        // Step by Step
        const path = searchAlgo.step_start(blocks, 
            blocks[source.x][source.y],
            blocks[target.x][target.y]);
...
function initInteractive() {
    ...
    loadBtn.mousePressed(() => {
        if (savedBlocks) {
            ...
            blocks = backupBlocksArr(savedBlocks);
...
function backupBlocksArr(arrAlt) {
    const usedArr = arrAlt ? arrAlt :blocks;
    const newBlockArr = [];

    for (const arr of usedArr) {
        newBlockArr.push([]);
